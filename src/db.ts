/**
 * SQLite database access layer for the CBI regulatory MCP server.
 *
 * Schema mirrors the Central Bank of Ireland regulatory structure:
 *   - sourcebooks        — top-level CBI regulatory codes and guidance documents
 *   - provisions         — individual rules, guidance notes, and requirements
 *   - enforcement_actions — CBI enforcement actions and settlement agreements
 *
...
export function getProvision(
  ...
  // Fuzzy fallback: try case-insensitive reference match
  return (
    (db
      .prepare(
        "SELECT * FROM provisions WHERE sourcebook_id = ? AND UPPER(reference) = UPPER(?) LIMIT 1",
      )
      .get(sourcebook.toUpperCase(), reference) as Provision | undefined) ??
    null
  );
...
export function checkProvisionCurrency(reference: string): {
  ...
  const row = db
    .prepare(
      "SELECT reference, status, effective_date FROM provisions WHERE UPPER(reference) = UPPER(?) LIMIT 1",
    )
    .get(reference) as
